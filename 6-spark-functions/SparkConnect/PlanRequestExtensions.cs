...
public static class PlanRequestExtensions
{
    public static async Task<Relation> 
        Execute(this ExecutePlanRequest executePlanRequest, SparkSession session)
        {
       
        executePlanRequest.Tags.Add("Example Tag");

        if (executePlanRequest.Plan.Root.Common == null)
        {
            executePlanRequest.Plan.Root.Common = new RelationCommon()
            {
                SourceInfo = "program.cs line 100"
            };
        }

        executePlanRequest.Plan.Root.Common.PlanId = session.GetNextPlanId();

        var response = session.Client.ExecutePlan(executePlanRequest, session.Headers);
